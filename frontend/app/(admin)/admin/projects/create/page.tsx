...
export default function CreateProjectPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!form.title || !form.overview || !content || !form.image_uri) { setError("Title, overview, content, and image URI are required."); return; }
        try {
            setLoading(true);
            setError(null);
    ...
    return (
        <FormCard title="New Project" description="Fill in the details below. The content will be uploaded as a Markdown file." onSubmit={handleSubmit} loading={loading} backHref="/admin/projects" error={error}>
            ...
            <FormField label="Cover Image URI" placeholder="e.g. images/project-cover.png" value={form.image_uri} onChange={set("image_uri")} hint="S3 key or URL for the cover image." required />
