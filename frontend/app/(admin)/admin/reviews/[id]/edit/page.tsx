...
export default function EditReviewPage() {
    ...
    useEffect(() => {
        repo.getReviews().then(reviews => {
            const r = reviews.find(r => r.id === id);
            if (!r) { setError("Review not found."); return; }
            setForm({ client_name: r.client_name, client_title: r.client_title, message: r.message, client_image_uri: r.client_image.uri, client_image_alt: r.client_image.alt_text, company_image_uri: r.company_image.uri, company_image_alt: r.company_image.alt_text });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await repo.updateReview(id, { ...existing, client_name: form.client_name, client_title: form.client_title, message: form.message, client_image: { uri: form.client_image_uri, alt_text: form.client_image_alt, url: form.client_image_uri }, company_image: { uri: form.company_image_uri, alt_text: form.company_image_alt, url: form.company_image_uri } });
    ...
    return (
        <FormCard title="Edit Review" onSubmit={handleSubmit} loading={loading} backHref="/admin/reviews" error={error} submitLabel="Update Review">
            ...
            <FormTextarea label="Review Message" value={form.message} onChange={set("message")} rows={4} required />
